...
| Scientific validation | source provenance and simulation limitations are explicit; the [one-subject AddBiomechanics handoff](ADDBIOMECHANICS_VALIDATION_HANDOFF_20260915.md) now verifies artifact hashes, adult-male scope, disjoint fit/held-out tables, residual metrics and the canonical prediction clock; the [acquisition receipt](ADDBIOMECHANICS_SUBJECT_ACQUISITION_20260915.md) binds one real adult male and four measured gait/stair reference tables | partial | no Numi prediction comparison or held-out residual is supplied; no subject calibration, population variability, uncertainty, or clinical validation | benchmark protocol with held-out anatomical and mechanical data; claims limited to measured scope |
