...
| Scientific validation | source provenance and simulation limitations are explicit; the [one-subject AddBiomechanics handoff](ADDBIOMECHANICS_VALIDATION_HANDOFF_20260915.md) now verifies artifact hashes, adult-male scope, disjoint fit/held-out tables, residual metrics and the canonical prediction clock; the [acquisition receipt](ADDBIOMECHANICS_SUBJECT_ACQUISITION_20260915.md) binds one real adult male and four measured gait/stair reference tables; the [subject binding](ADDBIOMECHANICS_SUBJECT_BINDING_20260915.md) joins that source to the current evidence graph and retains its mass mismatch | partial | no Numi prediction comparison or held-out residual is supplied; the current 97.13195176621342 kg rigid-body owner does not match the 65.5 kg measured subject, and no subject scaling, population variability, uncertainty, or clinical validation exists | benchmark protocol with held-out anatomical and mechanical data; claims limited to measured scope |
