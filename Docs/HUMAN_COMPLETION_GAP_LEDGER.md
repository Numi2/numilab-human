...
| Joint Brain–Human–Matter runtime | authored Matter, prepared q/v/muscle state and source-compliant NHEQ2/NHLIM1 rows enter the native v7 coupled solve; source oracle, Metal force/tangent/factor and bounded joint-transaction checks pass with the existing convergence tolerance | partial | initial-pose recovery and prepared muscle numerics have historical 6.4 ms replay/dropout; compensated candidate/accepted geometry now passes 100/50/25 µs replay over a common 1.6 ms ([receipt](ACCEPTED_STATE_PRECISION_20260913.md)); 12.5 µs clock migration and full source-force convergence remain open; anatomical package lowering, mass/active-force replacement and standing control remain incomplete; bounded accepted time is not sustained behavior | complete source-bound HumanPack/CompiledRun, constrained coupled physics, accepted-only sensing/control and frozen standing/walking qualification |
...
collection. The full refinement gate still needs exact 12.5 µs transaction
clocks and source-force convergence; behavioral telemetry still lacks complete
...
0.3132 N and 0.3896 N, so full temporal/source-force convergence, the 12.5 µs
clock path and sustained behavior remain open. These engineering receipts do
not close anatomical supports/loading, source activation, two-way blood mass
and momentum transfer, unresolved material data or calibration.
