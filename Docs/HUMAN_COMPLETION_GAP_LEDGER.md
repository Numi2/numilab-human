...
The current repository evidence layer is published at `fbad284` on `main` and
...
[`current-human-evidence-join/receipt-v5.json`](media/current-human-evidence-join-20260915/receipt-v5.json).
...
of source compiled rigid-body mass. This closes source mass ownership and
non-duplication bookkeeping while leaving anatomical organ/blood/fat/skin and
soft-tissue mass, material, subject, standing, recovery, and walking gates open.
The registered-foot follow-on now emits 30 conservative body-frame proxy
enclosures for the four target foot bodies
...
count remains zero. The new cross-domain source/evidence bridge
([receipt](HUMAN_SOURCE_EVIDENCE_BRIDGE_20260915.md)) now binds that extension
...
[`gap-execution/report-v73.json`](media/gap-execution-20260915/report-v73.json).
...
The same bridge now binds the source-compliant non-maximal recruitment
...
leaves `--muscle-activation` unspecified, so the native probe uses its
qualified source default `0.8` recruitment ceiling and emits the per-route
recruitment state. This removes the historical `0.5` negative control from
the normal standing path; it does not promote standing because the exact-clock
...
The new [one-subject AddBiomechanics validation handoff](ADDBIOMECHANICS_VALIDATION_HANDOFF_20260915.md)
adds a source-bound compiler for the remaining subject-calibration path. It
requires a hashed subject-level artifact, one recorded adult male, disjoint
calibration and held-out trials, same-grid reference/prediction tables,
declared joint-angle/GRF/moment thresholds, and an exact `12.5 us` prediction
clock. It reports residuals and activity coverage without downloading the
aggregate archive or promoting any mechanics, activation, contact, material,
blood-transfer, standing, recovery, walking, or release claim. No real
AddBiomechanics subject artifact was bound by that handoff, so the remaining
prediction comparison and scientific validation gates remain open.

The follow-on [subject acquisition increment](ADDBIOMECHANICS_SUBJECT_ACQUISITION_20260915.md)
now binds the real Falisse2017 `subject_1` artifact: a 43-year-old male,
1.78 m, 65.5 kg, with a verified raw SHA-256 and four extracted gait/stair
reference tables for angles, vertical ground reaction, and right-knee moment.
The immutable receipt records that this source has walking trials but no
standing or recovery trial. This closes acquisition and measured-reference
provenance for one subject; no Numi prediction comparison, held-out residual,
activation fit, anatomical support/load calibration, material identification,
organ/blood/tissue/fat mechanical owner, or standing/walking claim is promoted.

The [subject binding increment](ADDBIOMECHANICS_SUBJECT_BINDING_20260915.md)
now joins that receipt to the current Human evidence graph and the compiled
MyoSim rigid-body owner. It exposes a `31.63195176621342 kg` difference between
the measured `65.5 kg` subject and the current `97.13195176621342 kg` rigid-body
mass, and therefore keeps `same_scaled_mechanical_subject=false`. The mismatch
is retained as a required mass/inertia reconstruction blocker; it is not
silently treated as calibration. Prediction comparison, standing/recovery
reference coverage, activation, materials, anatomical loading, organ/blood/
tissue/fat ownership, standing, recovery, and walking remain open.

The [subject mass-scaling candidate](SUBJECT_MASS_SCALING_CANDIDATE_20260915.md)
now emits deterministic rows for all 103 rigid bodies. A uniform factor of
`0.6743404081661175` closes the scalar target at `65.5 kg` while preserving the
source tree, but it assumes fixed geometry and scales inertia with mass. It is
therefore a handoff for the native subject-scaled solve, not segment
composition, geometry, inertial, material, organ, blood, fat, muscle, standing,
or walking qualification.

The [subject-scaled runtime handoff](SUBJECT_SCALED_RUNTIME_INPUT_20260915.md)
now lowers that candidate into the published `NHRIGID2` layout, verifies all
103 source-to-157-engine mappings, and closes the serialized binary mass at
`65.50000002491288 kg`. The physical [native replay](SUBJECT_SCALED_NATIVE_REQUALIFICATION_20260915.md)
consumes that exact binary
with the public 416-route muscle/tendon/contact/equality tuple on the Apple M4
Pro: 64 steps at `12.5 us`, six active source support contacts, zero
penetration, no root assistance, and bitwise replay. It records
`0.146436423063 m/s2` peak acceleration and a `0.03299692183 N` dynamic
force-audit residual over `0.8 ms`; this is a bounded input-consumption
requalification, not full generalized force convergence. Segment composition,
...
The six-vessel mass-moment candidate is now hash-bound to the registered
source surfaces and this native regional receipt. Its exact-clock transport
probe advances first and second spatial moments for six owners over 512
attempts (511 accepted, one atomically rejected), conserving candidate mass
and linear momentum with bitwise replay. It remains a surface-integral
engineering probe: no anatomical lumen, mechanical blood-mass owner,
...
The [common-duration refinement](NATIVE_PASSIVE_STAND_REFINEMENT_20260915.md)
now runs the same passive source state for `6.4 ms` at 100, 50, 25 and
12.5 microseconds. Static normalized residual RMS is identical at
`6.42342632457e-6`, zero penetration and bitwise replay hold for every case,
but the release peaks are `0.642376363277`, `0.682456016541`,
...
The current physical requalification is recorded in
[NATIVE_STAND_DYNAMIC_HANDOFF_20260914.md](NATIVE_STAND_DYNAMIC_HANDOFF_20260914.md).
It supersedes older equivalent-horizon numbers for the isolated 72be18e9 source
branch: static support, fibre handoff, and source/runtime support parity pass,
while the exact-clock dynamic release remains a hard failure. The receipt keeps
...
| Muscle actuation | all 416 current-pose routes, wraps, activation, compliant fibre/tendon equilibrium, `J^T`, persistent Metal stepping; exact ABI 3 first-order activation hold with ABI 2 compatibility and native 416-muscle/trajectory receipt ([receipt](ACTIVATION_EXACT_20260913.md)); paired full/suffix/terminal/hood geometry, exact-view admission and prepared-source comparisons ([receipt](SOURCE_ROUTE_PRECISION_20260913.md)); the [soft-tissue surface candidate](SOFT_TISSUE_SURFACE_CANDIDATE_20260914.md) binds 416 source routes to 148 muscle and two tendon surface identities while retaining 238 unbound routes; the [activation/recruitment candidate](ACTIVATION_RECRUITMENT_CANDIDATE_20260914.md) binds the source-compliant one-male pose, support sharing, 416-route recruitment and fibre state while rejecting uniform activation-1; the [native equilibrium audit](NATIVE_STAND_EQUILIBRIUM_AUDIT_20260914.md) records the complete static per-DoF reaction certificate and the corrected [native preload requalification](NATIVE_STAND_PRELOAD_REQUALIFICATION_20260914.md); the [960-sweep all-DoF requalification](NATIVE_WHOLE_BODY_ALL_DOF_REQUALIFICATION_20260915.md) confirms root balance and bitwise replay but leaves the internal normalized residual at `0.141417024366` with `internal_balanced=false`; the [activation-cap requalification](NATIVE_ACTIVATION_CAP_REQUALIFICATION_20260915.md) shows a 0.8 cap is equivalent while a 0.5 cap worsens the residual to `11.8273303596`, so a global cap is not a standing solution; the [geometric muscle-volume candidate](MUSCLE_GEOMETRIC_VOLUME_CANDIDATE_20260915.md) binds 60 single-closed source muscle volumes to immutable member identities while retaining FEM, density, mass, material, active-force and calibration ownership false; the [route-volume incidence candidate](MUSCLE_ROUTE_VOLUME_JOIN_CANDIDATE_20260915.md) joins those 60 closed geometry identities to 78 routes across 82 shared incidences while retaining the 238 unbound routes; the [body composition integration candidate](BODY_COMPOSITION_INTEGRATION_CANDIDATE_20260914.md) joins the 378 organ identities, 329 blood-transport members, 150 muscle/tendon surface identities, the exact BodyParts3D FJ2810 skin shell, and exact 12.5 µs physiology clock while retaining all physical volume, mechanical, material, and calibration owners unresolved; the v14 body-composition join additionally binds the six-vessel spatial mass-moment transport candidate and the compiled MyoSim rigid-body mass owner while retaining all anatomical and soft-tissue, fat, material, and activation-calibration owners unresolved. | partial | inferred compliant architecture and bounded recruitment are not full OpenSim-equivalent dynamic fibre/tendon state; 238 routes have no emitted surface; 88 muscle surfaces remain non-admitted; fat geometry, skin physical volume, active volumetric force ownership, and activation calibration remain open | source curve/path/moment-arm parity plus registered force-transmitting tissue, held-out force-length-velocity, activation and dynamic state tests |
...
| Tendon and fascia continuum | live `NHFASC4` owns six bilateral pectoral, six latissimus-aponeurosis, eight external-oblique, and six internal-oblique routes in 12 transversely isotropic Matter objects; exact route directions, bilateral/class grouping, same-command-buffer endpoint-share replacement, per-step fixed-node reactions, replay, rollback, and current Apple M4 Pro one/four-step execution are gated | partial | the regional sheets are generated mechanics envelopes; homogeneous group frames are not per-tetrahedron histology; the matrix/fibre split and 10% load ownership are not independently calibrated; the captured contact-certification exhaustion is fixed and the updated runtime passes 4/8/16/64 steps with physical Human rollback, but 64 steps represent only 0.64 ms and take 86.95 seconds in the coupled transaction; fascia sliding/contact and sustained trunk loading are absent; there is no whole-body tendon/fascia continuum | registered regional meshes, spatial fibre fields and independent biaxial calibration, nonduplicated two-way load coupling, fascia interaction, convergence, sustained replay, and held-out deformation |
| Ligaments, cartilage, and menisci | exact bilateral BodyParts3D ribs 1--7 costal-cartilage FEM plus exact Open Knee oks003 ACL/PCL/MCL/LCL/PTL/QAT topology; `NHKNEE1` ABI 2 admits the six source fibre axes, material constants, in-situ stretches, and FeBio hash; Apple M4 Pro executes the neutral-stretch source-directed ligament/PTL law with three-body reactions, rollback, and bitwise replay | partial | the live reduced knee owner remains a smooth approximation; the separate source exp-linear law passes scalar/PCL prescribed-boundary checks but LCL source prestress rejects stage 1; final in-situ stretch failed a bounded one-step nonlinear/performance gate and needs staged equilibrium initialization; QAT volumetric activity, production cartilage/meniscus contact, calibration, and loaded flexion remain open; costal material remains an uncalibrated population prior; the body-composition v3 receipt now binds the one-plug `oks003` PTC calibration candidate while retaining native, stress-free-reference, population, and whole-human material gates open; registered mass-conserving v5 admission is implemented and the current native source-bound requalification passes ([receipt](COSTAL_TISSUE_REQUALIFICATION_20260913.md)), with loaded thorax mechanics unqualified | staged source prestress equilibrium, exact or validated reduced fibre-law parity, loaded knee contact/flexion, live non-duplicated costal coupling, calibration, and held-out joint-level outcomes |
| Anatomical collision and contact | source-bound ten-primitive plantar ellipsoid candidate from the authored NHCNT1 witnesses and paired NHBONES1 frames; isolated native Mac mini active-set fit closes body weight and floating-root wrench with bitwise replay ([v1 receipt](media/anatomical-support-candidate-20260915/native-static-support-receipt-v1.json)); the follow-up bounded pose-search owner keeps the geometry-certified unilateral manifold while recruitment improves internal normalized residual RMS to `0.71909649283` ([v2 receipt](media/anatomical-support-candidate-20260915/native-static-support-receipt-v2.json)); paired v2 all-128-DoF force artifacts reconstruct all six source rows and retain the internal leaders ([ledger](media/anatomical-support-candidate-20260915/native-support-force-ledger-v2.json)) | partial | internal generalized equilibrium remains false: `v_006` is `-20.4440 N` / `-2.5236 m/s²` and the ledger closure ratio is 1.0; dynamic contact, calibrated friction/compliance, swept-motion bounds, sustained loading, standing, recovery, and walking remain open | source-bound anatomical colliders, calibrated contact/material receipts, and held-out support/contact outcomes |
| Skin and exterior | the [full-skin shell candidate](SKIN_SHELL_CANDIDATE_20260914.md) now binds exact BodyParts3D `FJ2810` outer-shell identity to 185 source-bound anchors and 86 registered visual influences with sub-micrometre rest reconstruction, and the [native skin-shell admission](SKIN_SHELL_NATIVE_ADMISSION_20260914.md) proves four positive-pixel Apple M4 Pro views through the native 157-body renderer | partial | no shell thickness, physical volume, mechanical mass, constitutive material, collision/contact, self-contact, muscle sliding, adipose geometry, or deformation qualification; fat source remains absent | articulated skin/fat/fascia coupling with calibrated thickness/material, contact, volume control, visual and mechanical validation |
| Organs, vessels, and nerves | [source inventory](ORGAN_GEOMETRY_INVENTORY_20260913.md) now resolves the 18 exact BodyParts3D regions to 378 unique members across 386 declared memberships, with exact seam quotients and seven retained source-topology defects; [source-frame moments](ORGAN_GEOMETRY_MOMENTS_20260913.md) compute positive volume and first/second moments for 357 single closed members while retaining 14 disconnected candidates; the new [conservative component moments](ORGAN_GEOMETRY_COMPONENT_MOMENTS_20260915.md) additionally integrates seven disconnected members whose closed component bounds are disjoint, leaving seven overlapping-bound candidates and seven topology-defective members explicit; the [organ-to-blood cavity bridge](ORGAN_BLOOD_CAVITY_BRIDGE_20260913.md) hash-binds all four cardiac cavity members to unchanged CVSim hydraulic references and records their volume ratios; the follow-up [cardiac cavity body-link receipt](ORGAN_CARDIAC_CAVITY_BODY_LINK_20260913.md) closes their named MyoSim torso source/core frame bookkeeping while retaining physical-volume and blood-mass ownership false; the [cardiac cavity ownership receipt](ORGAN_CARDIAC_CAVITY_OWNERSHIP_20260913.md) now binds both exact disjoint-interior right-heart candidates to that bridge and body frame without selecting a biological interface or assigning mass; the new [cardiac blood-mass candidate](CARDIAC_BLOOD_MASS_CANDIDATE_20260914.md) binds both geometry conventions to all four CVSim hydraulic chambers and conserves a `0.39974235600733804 kg` zeroth-moment budget at an explicitly unresolved `1060 kg/m³` density while retaining physical-volume, mechanical-mass, and tissue-exchange owners null; the new [tissue mass composition candidate](TISSUE_MASS_CANDIDATE_20260914.md) covers all 18 regions and 378 unique source members, admits 342 single-closed organ-parenchyma candidates at an explicit unresolved density, and rejects eight shared seam members from double counting; the [component-sum tissue candidate](TISSUE_MASS_COMPONENT_CANDIDATE_20260915.md) adds seven safe disconnected members for 349 deterministic zeroth/first/second mass moments while retaining zero physical owners; the [regional native tissue-mass candidate](REGIONAL_TISSUE_MASS_CANDIDATE_20260914.md) now binds the 14-region, 13,516-node costal partition to the single-male source, conserves the published 0.11369939548001184 kg tissue plus residual torso mass within the pinned serialization tolerance, and retains production ownership false; the [body composition integration candidate](BODY_COMPOSITION_INTEGRATION_CANDIDATE_20260914.md) hash-joins this inventory to the seven-bed blood/oxygen candidates, 416-route muscle identity graph, and source-bound FJ2810 skin shell, and its immutable v4 receipt adds the 24-region Rodero cardiac-wall source/config identity pair without assigning a physical owner; the [cardiac material/activation/support/loading contract](CARDIAC_LOADING_CONTRACT_20260913.md) preserves supplied source values and six explicit unresolved admission gates; selected torso surfaces remain pose-bound visual layers; the [torso organ/body-link receipt](ORGAN_TORSO_BODY_LINK_20260913.md) now closes exact source/core body-frame bookkeeping for all five selected organ surfaces, six selected vessel surfaces, and the spinal-cord surface; the physiology compiler preserves overlap; the new [six-vessel source-frame registration](ORGAN_VESSEL_REGISTRATION_20260913.md) hash-binds exact vessel surfaces and transforms their source moments into the pinned MyoSim world frame; the follow-up [exact vessel body-link receipt](ORGAN_VESSEL_BODY_LINK_20260913.md) validates all 103 NHRIGID2 source-body mappings and closes named source/core body-frame bookkeeping for the six vessel surfaces; the physical-M4 [native vessel-registration admission](NATIVE_VESSEL_REGISTRATION_20260913.md) passes its focused 8/8 compiler/Metal/fullbody selection while retaining mechanics and calibration false; the ABI38 native owner path requires an explicit compartment, density, normalized FEM region, and cooked reference spatial moments for any mechanical blood mass ([receipt](BLOOD_MASS_OWNER_20260913.md)); the source-to-native four-chamber identity/owner admission now has a physical-M4 receipt ([receipt](NATIVE_ORGAN_BLOOD_BINDING_20260913.md)); the ABI38 source contract now has a physical-M4 synthetic [pressure-gradient wall reaction](PRESSURE_GRADIENT_REACTION_20260913.md) with a coupled hydraulic-to-FEM Jacobian; the ABI39 native increment adds an explicit finite-volume [fluid-momentum owner](FLUID_MOMENTUM_TRANSFER_20260913.md) with physical-M4 force conservation and Jacobian evidence; the current native source/material/cavity selection requalifies 17/17 tests at the post-clock physical M4 Pro commit ([current receipt](ORGAN_BLOOD_CAVITY_BRIDGE_20260913.md)); the real 157-body/129-q/128-v package now also passes [fullbody vascular owner admission](FULLBODY_VASCULAR_ADMISSION_20260913.md) with two compartments, one inertial edge, one normalized FEM owner, density and explicit momentum-transfer metadata, and bitwise package replay; the Human source bridge remains unqualified for body-link vessel mechanics and subject transfer; the v14 body-composition join now binds the six source-vessel mass-moment owners, the exact-clock conservative spatial transport probe, and the compiled MyoSim rigid-body mass owner; lumen, mechanical blood mass, tissue transfer, materials, and subject calibration remain open.| open | native pressure/volume/FEM-wall coupling now has [synthetic qualification](CARDIAC_WALL_COUPLING_20260912.md); the source moments, including the [component-sum receipt](ORGAN_GEOMETRY_COMPONENT_MOMENTS_20260915.md), bridge, and loading contract remain source-frame/parameter evidence and do not union, register, assign a physical volume, recover unloaded anatomy, or assign closure materials; the torso body-link receipt and narrower exact body-link receipts close named source/core frame bookkeeping only and do not create a tube, lumen, area, material, density, or mechanical owner; the owner proves bounded reference moments, initial FEM mass partition, dynamic-volume correction, co-moving inertia, a synthetic [time-integrated moment/pressure-impulse/atomic-restore subgate](BLOOD_MOMENTUM_RESTORE_20260913.md), an explicit pressure-reaction path, and a synthetic explicit fluid-momentum reaction, not dynamic organ or vessel mechanics; [fullbody vascular package admission](FULLBODY_VASCULAR_ADMISSION_20260913.md) is now proved as a wiring/replay subgate; the follow-up [coupled-support requalification](COUPLED_SUPPORT_REQUALIFICATION_20260913.md) passes a synthetic exact-clock vascular root with all ten support receptor rows at fixture pressure `0.0008`, and a deliberate `0.001` pressure case fails closed on a materially negative accepted impulse; the physical-M4 [ABI4 source-support-force receipt](NATIVE_SUPPORT_FORCE_20260913.md) now transports the six active NHCNT2 source reactions into the exact-clock stand owner and passes source balance, contact-disabled one-step parity, deterministic 64-step transaction, and bitwise replay; the [declared-bound horizon increment](NATIVE_SUPPORT_FORCE_HORIZON_20260913.md) raises native CLI admission to 4096 steps and completes a deterministic 512-step/6.4 ms run while exposing temporal drift (`114347.445 m/s²`, `1.4058` velocity delta, `0.0047894` configuration delta) ([receipt](media/native-support-force-horizon-20260913/receipt.json)); the [native equilibrium audit](NATIVE_STAND_EQUILIBRIUM_AUDIT_20260914.md) adds a complete static per-DoF certificate but retains the dynamic release handoff as failed; the current receipts explicitly retain anatomical supports/loading, activation calibration, unresolved material data, subject calibration, and force convergence as unqualified; calibrated anatomical organ FEM/MPM, vessel tube and neural mechanics remain open | named volumetric/tubular models, calibrated materials/boundaries, conservation, contact, and replay evidence |
| Systemic physiology | source-bound HumanPack physiology, passive blood-volume/flow/species and fixed organ-reservoir exchange in the existing Matter transaction ([requalified receipt](ORGAN_CIRCULATION_REQUALIFICATION_20260913.md)); the new bounded [accepted physiology-step owner](PHYSIOLOGY_STEP_20260913.md) now executes conservative flow/species exchange and rejected-candidate rollback on the synthetic graph; the bounded [explicit blood-mass transfer owner](BLOOD_MASS_TRANSFER_STEP_20260913.md) now binds one owner per fixture compartment/tissue reservoir, advects mass through flow, exercises both exchange directions, and rolls hydraulic/mass state back atomically; the bounded [pinned Shi/Hose cardiac source step](SHI_HOSE_CARDIAC_STEP_20260914.md) now evaluates retained source activation, elastance/compliance pressure, one-way valve and resistance/inertance flow, conservative compartment volume, and accepted-step rollback on the ten-compartment source graph; the new [CVSim21 aggregate blood-mass step](CVSIM21_BLOOD_MASS_STEP_20260914.md) binds all 21 pinned source volume owners to an explicit unresolved-density candidate, conservatively advects source mass, and restores mass and hydraulic state atomically at the exact 12.5 µs clock; the [cardiac cavity blood-mass candidate](CARDIAC_BLOOD_MASS_CANDIDATE_20260914.md) binds both right-heart geometry alternatives to all four hydraulic chambers and preserves a common zeroth-moment budget under the explicit unresolved density; the new [regional organ blood transport candidate](ORGAN_BLOOD_TISSUE_TRANSPORT_20260914.md) partitions six CVSim21 source blood owners across seven lung, kidney, stomach, pancreas, and liver beds and passes exact-clock zeroth-moment transport with atomic rollback while retaining vessel, tissue-exchange, spatial-moment, and calibration gates; the new [regional tissue oxygen exchange candidate](ORGAN_TISSUE_EXCHANGE_CANDIDATE_20260914.md) attaches a conserved, bidirectional oxygen amount to those same seven beds and passes exact-clock advection, exchange and rejected-step neutrality while retaining capillary, gas, metabolic and calibration gates; the [body composition integration candidate](BODY_COMPOSITION_INTEGRATION_CANDIDATE_20260914.md) preserves the shared bed/member identity and 12.5 µs clock across organ, blood, tissue, and muscle source layers while keeping anatomical transfer and calibration open; full Shi–Hose four-chamber systemic/pulmonary source drive and one-way valves pass ten physical M4 Pro cycles, timestep refinement, independent FP64 comparison and clock/transaction checks ([cardiac receipt](CARDIAC_SOURCE_REPRODUCTION_20260912.md)); ABI38 supplies a conservative explicit blood owner, reference first/second spatial moments, initial FEM mass partition, dynamic-volume correction, co-moving inertial transfer, production dynamic-moment evaluation, bitwise replay, and the synthetic [time-integrated moment/closed-surface pressure-impulse/atomic-restore audit](BLOOD_MOMENTUM_RESTORE_20260913.md)); ABI39 adds a synthetic explicit fluid-momentum reaction owner with a coupled force/Jacobian check ([receipt](FLUID_MOMENTUM_TRANSFER_20260913.md)); the current native organ/blood selection requalifies the associated source/engineering physiology, circulation, material and cavity checks 17/17 on the physical M4 Pro; the v14 body-composition join now binds the six-vessel spatial mass-moment transport probe, the physical-M4 regional exchange receipt, and the compiled MyoSim rigid-body mass owner; it remains a candidate transport/integration subgate without anatomical perfusion or tissue-side transfer qualification. | partial | anatomical template retains 276 unresolved parameters; Shi–Hose C·P storage lacks absolute dilution volumes; separate [CVSim21](CVSIM21_REGIONAL_CIRCULATION_20260912.md) supplies a 21-compartment 5,150 mL aggregate blood model with native refinement/ten-cycle evidence and four cavity references, and its new mass owner closes only source aggregate zeroth-moment transfer with an unresolved density candidate; two disjoint geometric RA/RV overlap candidates now exist, but biological interface selection remains open; [native deforming-wall coupling](CARDIAC_WALL_COUPLING_20260912.md) passes a hollow-shell case; the current Human source now has source/world and named body-frame vessel bookkeeping plus the four cardiac cavity torso links, but still lacks vessel tubes/areas, anatomical tissue-side exchange, calibrated density/materials, gas/metabolism/thermal state and held-out physiology, so full anatomical perfusion/wall feedback and two-way blood/tissue transfer remain unqualified | sourced models and parameters, conservative multi-rate coupling, physiological observables and exact-stack replay with uncertainty |
| Balance, control, and gait | transactional part coactivation and compiled standing transaction; mass-coupled offline source-stop reactions with equality tangent and recruitment derivative pass; coupled offline recruitment reaches 0.00814 at 1,024 activation sweeps/48 posture updates, with bitwise replay and independent source mass/gravity/reaction checks ([receipt](COUPLED_EQUILIBRIUM_20260908.md)); historical 420-trial regression evaluator and schema; [single-male capability contract](SINGLE_MALE_COMPLETION.md); the exact-clock persistent native stance now has a four-grid, assistance-free refinement receipt ([receipt](EXACT_STAND_REFINEMENT_20260913.md)); native source joint-limit projection now completes the 12.5 µs/64-step transaction with one-step FP64 parity and bitwise replay ([receipt](EXACT_STAND_LIMIT_PROJECTION_20260913.md)); the ABI4 physical-M4 source-support-force run adds six active NHCNT2 loads, static `balanced=true`, `952.864475176 N` total support, `1.85e-5` one-step velocity parity and bitwise 64-step replay; the [long-horizon receipt](NATIVE_SUPPORT_FORCE_HORIZON_20260913.md) completes 512 exact 12.5 µs steps but reaches `114347.445 m/s²` peak acceleration, `1.4058` velocity delta, and `0.0047894` configuration delta ([receipt](media/native-support-force-horizon-20260913/receipt.json)); the [native equilibrium audit](NATIVE_STAND_EQUILIBRIUM_AUDIT_20260914.md) records the complete static per-DoF certificate and the corrected [native preload requalification](NATIVE_STAND_PRELOAD_REQUALIFICATION_20260914.md); internal balance remains false and the exact-clock dynamic release remains failed | partial | bounded FP64 offline `balanced=true` against 0.05; source-compliant FP64 preparation and fixed FP32 pose/recruitment balance now pass ([receipt](SOURCE_COMPLIANT_EQUILIBRIUM_20260910.md)); the native limit-aware exact-clock stance lowers peak acceleration to `1.716e3 m/s²` and maximum velocity increment to `0.120 rad/s`, but retains `compiled_stand_balanced=false`, residual `0.8608`, and configuration drift over 0.8 ms; dynamic FP32 loaded equilibrium remains open; source-compliant dynamic limits now have bounded source/kernel/transaction checks ([receipt](SOURCE_DYNAMIC_LIMITS_20260908.md)); the native accepted-root producer now passes a four-root real joint run ([receipt](ACCEPTED_STATE_PRECISION_20260913.md)); complete behavioral audit/contact coverage and TaskPack lowering, prepared-stance loading, sustained posture control, deployable walking task, learned policy and held-out gait remain unqualified | assistance-free stable standing, perturbation recovery, registered foot contact, deterministic resets, and held-out gait metrics |
| Joint Brain–Human–Matter runtime | authored Matter, prepared q/v/muscle state and source-compliant NHEQ2/NHLIM1 rows enter the native v7 coupled solve; source oracle, Metal force/tangent/factor and bounded joint-transaction checks pass with the existing convergence tolerance; additive ABI8/v2 exact-clock admission now passes a real 12,500 ns physical substep and legacy entry remains fail-closed ([receipt](EXACT_CLOCK_20260913.md)); behavior compiler/telemetry now share the same canonical 12,500 ns clock and the physical-M4 focused selection passes 9/9 ([receipt](HUMAN_BEHAVIOR_CLOCK_20260913.md)); exact ABI 3 activation and 100/50/25 µs bounded replay are retained ([receipts](ACTIVATION_EXACT_20260913.md), [FORCE_CONVERGENCE_20260913.md)); the persistent native stand owner now records 100/50/25/12.5 µs equal-duration replay and support-wrench output ([receipt](EXACT_STAND_REFINEMENT_20260913.md)); the native source-limit owner now passes focused M4 regression and exact 12.5 µs/64-step replay ([receipt](EXACT_STAND_LIMIT_PROJECTION_20260913.md)); ABI4 source support-force transport and unilateral contact projection are committed and tested on the physical M4; the declared-bound CLI now admits 4096 steps and the accepted 512-step/6.4 ms run still shows temporal force drift ([receipt](NATIVE_SUPPORT_FORCE_HORIZON_20260913.md)); the executable per-DoF force ledger now retains all 128 coordinate rows and every required source contribution for pre-step diagnosis ([receipt](FORCE_LEDGER_20260914.md)) | partial | initial-pose recovery and prepared muscle numerics have historical 6.4 ms replay/dropout; the common-grid source applied-force difference increases as activation error halves, and the limit-aware persistent stand terminal still drifts with `compiled_stand_balanced=false`, so coupled state/force temporal convergence remains open; anatomical package lowering, mass/active-force replacement and standing control remain incomplete; behavior telemetry still reports physical_steps=0 and unknown audit coverage; bounded accepted time is not sustained behavior | complete source-bound HumanPack/CompiledRun, constrained coupled physics, accepted-only sensing/control and frozen standing/walking qualification |
| Apple runtime qualification | M4 Pro executes all routes, tendon transfers, rollback, and bitwise replay; the subject-scaled NHRIGID2 handoff is consumed by the public source tuple for a bounded 64-step 12.5 us replay with zero penetration and no root assistance ([receipt](SUBJECT_SCALED_NATIVE_REQUALIFICATION_20260915.md)) | partial | no same-workload performance qualification for the complete future tissue/contact/control stack; the bounded replay is not temporal convergence or sustained behavior | exact revision/artifact fingerprints, counters/traces, memory accounting, throughput, replay, and physical outcomes |
| Scientific validation | source provenance and simulation limitations are explicit; the [one-subject AddBiomechanics handoff](ADDBIOMECHANICS_VALIDATION_HANDOFF_20260915.md) now verifies artifact hashes, adult-male scope, disjoint fit/held-out tables, residual metrics and the canonical prediction clock; the [acquisition receipt](ADDBIOMECHANICS_SUBJECT_ACQUISITION_20260915.md) binds one real adult male and four measured gait/stair reference tables; the [subject binding](ADDBIOMECHANICS_SUBJECT_BINDING_20260915.md) joins that source to the current evidence graph and retains its mass mismatch; the [mass-scaling candidate](SUBJECT_MASS_SCALING_CANDIDATE_20260915.md) closes only the scalar target with an explicit fixed-geometry assumption; the [subject-scaled runtime input](SUBJECT_SCALED_RUNTIME_INPUT_20260915.md) verifies the 103-to-157 binary handoff and the [native requalification](SUBJECT_SCALED_NATIVE_REQUALIFICATION_20260915.md) consumes it on the physical M4 Pro with bounded 12.5 us replay | partial | no Numi prediction comparison or held-out residual is supplied; the uniform mass candidate still lacks segment composition, geometry/inertia calibration, population variability, uncertainty, clinical validation, full generalized force convergence, sustained standing, recovery, or walking | benchmark protocol with held-out anatomical and mechanical data; claims limited to measured scope |
...
The [organ/circulation owner](ORGAN_CIRCULATION_REQUALIFICATION_20260913.md) adds a freshly rebuilt source-bound passive owner receipt after the Human CLI source changed. Matter ABI26/package11 requires recooking earlier Matter payloads; the 12 September receipt remains historical and the current receipt binds the requalified source tree. Native numerical tests do not close anatomical mechanics, physiological calibration or standing/gait.
...
| Blood mechanics | ABI38 explicit one-owner-per-compartment registration, normalized FEM mass partition, cooked reference first/second spatial moments, dynamic-volume gravity correction, co-moving inertial transfer, production dynamic spatial-moment evaluation, the physical-M4-Pro [time-integrated moment/closed-surface pressure-impulse audit](BLOOD_MOMENT_CLOSURE_20260913.md), and the explicit [pressure-gradient wall reaction path](PRESSURE_GRADIENT_REACTION_20260913.md) now pass compiler, native Metal, package round-trip, Jacobian and replay checks; ABI39 adds a physical-M4 synthetic [finite-volume fluid-momentum reaction](FLUID_MOMENTUM_TRANSFER_20260913.md) with force conservation and a coupled tangent; the [six-vessel registration receipt](ORGAN_VESSEL_REGISTRATION_20260913.md) closes source-to-world frame bookkeeping and the [exact body-link receipt](ORGAN_VESSEL_BODY_LINK_20260913.md) closes named source/core body-frame bookkeeping; the Human source still lacks vessel direction/area, tubular wall data, mechanical blood mass, and tissue-side exchange, so two-way blood/tissue transfer, calibrated density, and organ mechanics remain open | mechanical density convention, vessel direction/area and tubular wall data, subject calibration |

Closure-label 11–17 passive coefficients, global ventricular embedding, the retained RV CSV discrepancy and held-out physiological measurements remain unresolved. No default values or synthetic trajectory closes these gates.
...
The [source equality integration](HUMAN_SOURCE_EQUALITIES.md) carries all 51
pinned scalar-joint rows into the same coupled physical solve. Its separate
private preconditioner preserves the source A0 reaction authority. The
[behavioral evaluator](HUMAN_BEHAVIOR_QUALIFICATION.md) fixes evidence admission
for all 420 trials, but has no native telemetry producer or qualified controller
yet. Coverage presence and synthetic validator fixtures do not close physical
or behavioral rows.
...
with exact runtime/input fingerprints, replay, and reaction audits. Rejected
ordinary Human steps now restore q/v, muscle state, contact warm starts, and
accepted counters. The original failed runs remain available.

The next convergence gate is longer physical duration with timestep refinement,
registered support contact and closed-loop balance, plus resident allocation
and GPU profiling. The 64-step result spans only 0.64 ms, uses 86.95 seconds of
coupled execution, and peaks at 1.37 GB process memory; it does not qualify
sustained tissue loading, standing, gait, or interactive performance.
...
The [coupled offline equilibrium increment](COUPLED_EQUILIBRIUM_20260908.md)
closes the later FP64 internal stance gate at residual 0.00814. NHCNT2 geometry
and source mass/gravity/reaction checks pass. These results supersede the older
initial-equilibrium failures above, while loaded NHEQ2/NHLIM1 tissue admission,
source-compliant dynamics, calibration and sustained behavior remain open.

The [v7 prepared-state increment](PREPARED_STATE_ADMISSION_20260908.md) now admits
the identical FP32 stance with curved support and the small authored FEM fixture.
...
partial coverage. The [muscle numerical repairs](PREPARED_MUSCLE_NUMERICS_20260911.md),
...
The [accepted-coordinate and metric increment](ACCEPTED_STATE_PRECISION_20260913.md),
native `b0e195e1` / Brain `38c4b5c7`, repairs the retained 25 µs failure at the
original 16-iteration budget and 0.005 tolerance. The 100/50/25 µs trajectories
now complete 1.6 ms and replay all recorded state bitwise (224 accepted roots
including replay). Fifty selected native controls pass across the compatibility
run and focused fixture rerun; the failed attempts remain retained. A real
four-root joint runner emits four accepted metric samples and idempotent final
collection. The full refinement gate now has a bounded exact 12.5 µs
transaction-clock receipt, but still needs source-force and temporal
convergence; behavioral telemetry still lacks complete
audit/contact coverage, reset measurements, generic TaskPack lowering and full
accepted-root SHA proof. Neither execution-registry task is marked complete.

The [paired source-route increment](SOURCE_ROUTE_PRECISION_20260913.md), native
`53670294` with Brain `38c4b5c7`, extends precise geometry through full/suffix
muscle routes, terminal transfer and hood assembly. The actual 416-route
read-only paired evaluation passes unchanged path and same-path fibre gates;
maximum path error is 0.128 µm, and observed full source-force differences are
0.0575 N at 100 µs and 0.3788 N at 1 µs. The final stack again passes all
100/50/25 µs 1.6 ms trajectories with bitwise replay, 23 selected native checks
and the four-root production metric run. The captured sphere-wrap linkage
failure is retained and repaired. Common-grid force differences remain
0.3132 N and 0.3896 N, so full temporal/source-force convergence and sustained
behavior remain open. The exact-clock receipt is an admission and one-substep
runtime result, not a convergence or sustained-behavior result. These
engineering receipts do not close anatomical supports/loading, source
activation calibration and held-out data, two-way blood mass and momentum transfer, unresolved material data
or calibration.

The [explicit fluid-momentum owner increment](FLUID_MOMENTUM_TRANSFER_20260913.md)
is a separate ABI39 native subgate. It requires an authored pressure reaction,
one registered blood owner, and exactly one matching inertial vascular edge;
the physical M4 run passes the synthetic force, conservation, replay, and
coupled-Jacobian checks. The Human source bridge does not yet provide vessel
direction/area, body-frame registration, subject density, or a tissue-side
...
[SINGLE_MALE_COMPLETION.md](SINGLE_MALE_COMPLETION.md) supersedes the historical
420-trial population as a completion criterion. That evaluator and its receipts
remain regression evidence. The new capability compiler requires all 25 named
scenarios across seven families, disjoint calibration/validation sessions,
source-bound measurements and empirical margins. Compilation does not qualify
behavior, authenticate measurements, attest a freeze date, or select a measured
