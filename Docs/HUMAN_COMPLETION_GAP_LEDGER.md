...
| Organs, vessels, and nerves | selected exact BodyParts3D torso surfaces are pose-bound visual layers; the physiology compiler additionally binds 18 exact source regions and preserves overlap; the ABI38 native owner path now requires an explicit compartment, density, normalized FEM region, and cooked reference spatial moments for any mechanical blood mass ([receipt](BLOOD_MASS_OWNER_20260913.md)) | open | native pressure/volume/FEM-wall coupling now has [synthetic qualification](CARDIAC_WALL_COUPLING_20260912.md); the new owner proves bounded reference moments, initial FEM mass partition, dynamic-volume correction and co-moving inertia on a synthetic registered region, not dynamic organ mechanics; calibrated anatomical organ FEM/MPM, vessel tube and neural mechanics remain open | named volumetric/tubular models, calibrated materials/boundaries, conservation, contact, and replay evidence |
