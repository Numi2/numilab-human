...
| Blood mechanics | ABI38 explicit one-owner-per-compartment registration, normalized FEM mass partition, cooked reference first/second spatial moments, dynamic-volume gravity correction, co-moving inertial transfer, production dynamic spatial-moment evaluation, the physical-M4-Pro [time-integrated moment/closed-surface pressure-impulse audit](BLOOD_MOMENT_CLOSURE_20260913.md), and the explicit [pressure-gradient wall reaction path](PRESSURE_GRADIENT_REACTION_20260913.md) now pass compiler, native Metal, package round-trip, Jacobian and replay checks; pressure-driven fluid momentum, two-way blood/tissue transfer, anatomical registration, and organ mechanics remain open | mechanical density convention, anatomical body registration, vessel direction/area data and subject calibration |
