...
| Joint Brain–Human–Matter runtime | native v3 and Swift load authored Matter; the corrected private Human predictor, total support velocity, restoring Newton term and immutable support history pass nine native regressions and both eight-root Brain cases on M4 Pro | partial | source joint equalities, anatomical package lowering, mass/active-force replacement and standing control are not integrated; 0.8 ms of accepted time is not sustained behavior | complete source-bound HumanPack/CompiledRun, constrained coupled physics, accepted-only sensing/control and frozen standing/walking qualification |
