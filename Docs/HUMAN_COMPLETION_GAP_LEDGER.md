...
AddBiomechanics subject artifact is currently bound, so scientific validation
remains open.
...
| Scientific validation | source provenance and simulation limitations are explicit; the [one-subject AddBiomechanics handoff](ADDBIOMECHANICS_VALIDATION_HANDOFF_20260915.md) now verifies artifact hashes, adult-male scope, disjoint fit/held-out tables, residual metrics and the canonical prediction clock | partial | no real subject artifact is bound; no subject calibration, population variability, uncertainty, or clinical validation | benchmark protocol with held-out anatomical and mechanical data; claims limited to measured scope |
