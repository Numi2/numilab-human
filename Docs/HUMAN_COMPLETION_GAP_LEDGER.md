...
| Anatomical collision and contact | eight source capsules and two heel ellipsoids use current-pose native/Metal support queries; initial source geometry and wrench pass; independent support impulses now pass bounded loaded weight/friction, batched replay and arena-admission checks ([receipt](HARD_SUPPORT_KKT_20260908.md)); the [foot contact registration candidate](FOOT_CONTACT_REGISTRATION_CANDIDATE_20260914.md) now binds 60 exact source-local foot rows, 30 reviewed is_a members, four target foot bodies, seven-pose continuity and the six active support witnesses while retaining collider/contact calibration false | partial | BodyParts3D collider admission, collision exclusions, calibrated friction/compliance, and whole-body anatomical contact remain open | conservative registered proxies, material receipts, deterministic replay, and held-out support/contact outcomes |
