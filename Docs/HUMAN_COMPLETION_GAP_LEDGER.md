...
| Organs, vessels, and nerves | selected exact BodyParts3D torso surfaces are pose-bound visual layers; the physiology compiler additionally binds 18 exact source regions and preserves overlap; the ABI35 native owner path now requires an explicit compartment, density, and normalized FEM region for any mechanical blood mass ([receipt](BLOOD_MASS_OWNER_20260913.md)) | open | native pressure/volume/FEM-wall coupling now has [synthetic qualification](CARDIAC_WALL_COUPLING_20260912.md); the new owner proves only bounded zeroth-order gravity scatter; calibrated anatomical organ FEM/MPM, vessel tube and neural mechanics remain open | named volumetric/tubular models, calibrated materials/boundaries, conservation, contact, and replay evidence |
| Systemic physiology | source-bound HumanPack physiology, passive blood-volume/flow/species and fixed organ-reservoir exchange in the existing Matter transaction ([receipt](ORGAN_CIRCULATION_INTEGRATION_20260912.md)); full Shi–Hose four-chamber systemic/pulmonary source drive and one-way valves pass ten physical M4 Pro cycles, timestep refinement, independent FP64 comparison and clock/transaction checks ([cardiac receipt](CARDIAC_SOURCE_REPRODUCTION_20260912.md)); ABI35 now supplies a conservative explicit blood owner and bitwise body-force replay ([receipt](BLOOD_MASS_OWNER_20260913.md)) | partial | anatomical template retains 276 unresolved parameters; Shi–Hose C·P storage lacks absolute dilution volumes; separate [CVSim21](CVSIM21_REGIONAL_CIRCULATION_20260912.md) supplies a 21-compartment 5,150 mL aggregate blood model with native refinement/ten-cycle evidence and four cavity references; two disjoint geometric RA/RV overlap candidates now exist, but biological interface selection remains open; [native deforming-wall coupling](CARDIAC_WALL_COUPLING_20260912.md) passes a hollow-shell case; first/second blood moments, momentum transfer, anatomical perfusion/wall feedback, gas/metabolism/thermal state and held-out physiology remain unqualified | sourced models and parameters, conservative multi-rate coupling, physiological observables and exact-stack replay with uncertainty |
...
| Blood mechanics | ABI35 explicit one-owner-per-compartment registration and normalized FEM gravity scatter now pass compiler, native Metal, and replay checks; first/second moments and two-way momentum remain open | mechanical density convention, anatomical body registration and subject calibration |
