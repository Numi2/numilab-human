...
| Organs, vessels, and nerves | [source inventory](ORGAN_GEOMETRY_INVENTORY_20260913.md) now resolves the 18 exact BodyParts3D regions to 378 unique members across 386 declared memberships, with exact seam quotients and seven retained source-topology defects; selected torso surfaces remain pose-bound visual layers; the physiology compiler preserves overlap; the ABI38 native owner path requires an explicit compartment, density, normalized FEM region, and cooked reference spatial moments for any mechanical blood mass ([receipt](BLOOD_MASS_OWNER_20260913.md)) | open | native pressure/volume/FEM-wall coupling now has [synthetic qualification](CARDIAC_WALL_COUPLING_20260912.md); the new inventory is source-data admission only and does not union or register a volume; the owner proves bounded reference moments, initial FEM mass partition, dynamic-volume correction, co-moving inertia, and a synthetic [time-integrated moment/pressure-impulse subgate](BLOOD_MOMENT_CLOSURE_20260913.md), not dynamic organ mechanics; calibrated anatomical organ FEM/MPM, vessel tube and neural mechanics remain open | named volumetric/tubular models, calibrated materials/boundaries, conservation, contact, and replay evidence |
