...
int runPreparedPathReference(const char* rigidPath, const char* musclePath, const char* initialPath) {
    const LoadedRigid rigid = loadRigid(rigidPath);
    ...
    const auto diagnostics = metalrobo::runMetalArticulatedOperator(model, input, gpu,
        {.pointJacobiansOnly = true,
         .mujocoActivationTimestepSeconds = static_cast<float>(initial.timestepMicroseconds * 1e-6)});
    ...
    for (std::size_t i = 0; i < muscles.muscles.size(); ++i) {
        ...
        const double dt = static_cast<float>(initial.timestepMicroseconds * 1e-6);
    ...
    const bool passed = maximumError <= 2.0e-6;
...
int main(int argc, char** argv) {
    try {
        if (argc == 5 && std::string(argv[3]) == "--prepared-paths")
            return runPreparedPathReference(argv[1], argv[2], argv[4]);
        if (argc < 3 || argc > 7) {
            std::cerr << "usage: " << argv[0] << " <myosim-fullbody-core-reference.nhrigid> "
                      ...
                      << "[numi-human-tendon-endpoints.nhtendon] [--metal] "
                         ...
                         "[--equilibrium] | --prepared-paths <prepared.nhinit>\n";
            return 2;
        }
