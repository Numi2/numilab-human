...
for steps,dt in [(1,'0.0001'),(2,'0.0001'),(4,'0.0001'),(6,'0.0001'),(60,'0.00001')]:
 label=f'before-{steps}-{dt}';cmd=list(base);cmd[4]=str(out/label);cmd[cmd.index('--muscle-step-count')+1]=str(steps);cmd[cmd.index('--muscle-step-seconds')+1]=dt
 ...
 rows.append(row);(out/'before.json').write_text(json.dumps(rows,indent=2)+'\n')
