...
rows=[]
for steps,dt in [(1,'0.0001')]:
 label=f'seed-{steps}-{dt}';cmd=list(base);cmd[4]=str(out/label);cmd[cmd.index('--muscle-step-count')+1]=str(steps);cmd[cmd.index('--muscle-step-seconds')+1]=dt
 ...
 rows.append(row);(out/'seed.json').write_text(json.dumps(rows,indent=2)+'\n')
