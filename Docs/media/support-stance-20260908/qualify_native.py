...
for steps,dt in [(1,'0.0001'),(64,'0.0001')]:
 label=f'qualified-native-{steps}-{dt}';cmd=list(base);cmd[4]=str(out/label);cmd[cmd.index('--muscle-step-count')+1]=str(steps);cmd[cmd.index('--muscle-step-seconds')+1]=dt;cmd+=extras+['--camera-index','0']
 ...
 row={'runtime_revision':source_revision,'label':label,'command':cmd,'exit_code':p.returncode,'elapsed_seconds':time.monotonic()-start,'metrics':{k:v for k,v in kv.items() if k.startswith(('compiled_stand','source_support','persistent_stand','stand_','muscle_step_max'))}};rows.append(row);(out/'qualified-native.json').write_text(json.dumps(rows,indent=2)+'\n')
