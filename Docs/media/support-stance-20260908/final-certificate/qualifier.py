...
def assess(text: str, exit_code: int | None) -> tuple[dict, list[str]]:
    ...
    try:
        ...
        forces = [float(metrics[f"contact_{i}_normal_force_n"]) for i in range(10)]
...
def run(args):
    ...
    profile_path = Path(__file__).resolve().parents[2] / "config/myosim-support-stance.v1.json"
    ...
    with stdout.open("w") as out, stderr.open("w") as err:
        ...
        try:
            ...
            process.wait()
            code = None
    metrics, failures = assess(stdout.read_text(), code)
    for key, path in files.items():
