...
request=json.loads((base/('horizon-request.json' if label.startswith('horizon') else 'describe-request.json' if label.startswith('describe') else 'legacy-request.json' if label.startswith('legacy') else 'transaction-request.json')).read_text())
