...
DYNAMIC_SCHEMA = "numi.human.dynamic-handoff.v2"
...
def _dynamic_state(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(payload.get("completed_steps") == 0 and type(payload.get("completed_steps")) is int,
             "dynamic handoff must be captured before completing a persistent step")
    _require(payload.get("passive_bias_policy") == PASSIVE_BIAS_POLICY,
...
def audit(arguments: argparse.Namespace) -> int:
    ...
    static = _static_state(_read_text(source))
    ...
    if dynamic_payload is None:
        ...
        comparisons = {
            ...
            "driven_actuator_force": _comparison(
                static["driven_actuator_force_n"], dynamic["driven_actuator_force_n"],
                absolute_tolerance=thresholds["force_absolute_n"],
                relative_tolerance=thresholds["force_relative"],
            ),
        ...
        decomposition = _comparison(
            dynamic["source_total_actuator_force_n"], reconstructed,
            absolute_tolerance=thresholds["decomposition_absolute_n"],
            relative_tolerance=thresholds["force_relative"],
        )
        if not decomposition["passed"]:
            reasons.append("dynamic source force does not decompose into driven force plus excluded bias")
    ...
    state_parity = bool(
        comparisons and comparisons["activation"]["passed"] and comparisons["fiber_length"]["passed"]
    )
    ...
    complete = (
        state_parity and muscle_force_parity and generalized_force_parity
        and decomposition_closed and equilibrium_closed
    )
    receipt = {
        "schema": SCHEMA,
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--dynamic-snapshot", type=Path,
                        help="optional numi.human.dynamic-handoff.v2 JSON when not embedded in the log")
